...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mt-6">
      <p className="text-[11px] uppercase tracking-[0.2em] text-muted px-6 mb-2">
        {title}
      </p>
  ...
  return (
    <div className="min-h-svh pb-28">
      <header className="px-6 pt-8 pb-4">
        ...
        <h1 className="font-display text-4xl text-ink mt-1">{userName(user)}</h1>
      </header>
      ...
      <Section title="the pact">
        <Row to="/pact-edit" label="Current pact" hint="day N" />
        <Row to="/pact-archive" label="Pact archive" />
        <Row to="/habits-peek" label="Partner" />
      ...
      <Section title="developer (skeleton)">
        ...
        <Row to="/pact-day-50" label="Day 50 — success" />
